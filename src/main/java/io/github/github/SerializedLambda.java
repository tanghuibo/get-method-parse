...
class SerializedLambda implements Serializable {
    ...
    public static SerializedLambda resolve(SFunction<?, ?> lambda) {
        if (!lambda.getClass().isSynthetic()) {
            throw new GetMethodParseException("该方法仅能传入 lambda 表达式产生的合成类");
        }
        try (ObjectInputStream objIn = new ObjectInputStream(new ByteArrayInputStream(SerializationUtils.serialize(lambda))) {
            ...
            throw new GetMethodParseException("This is impossible to happen", e);
        }
    }
    ...
    public String toString() {
        ...
        return interfaceName.substring(interfaceName.lastIndexOf('.') + 1) + " -> " + implName.substring(implName.lastIndexOf('.') + 1) + "::" + implMethodName;
    }
}